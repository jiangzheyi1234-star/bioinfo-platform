        ...
        try:
            ...
            if blast_bin:
                ...
                if "dir" in out_dir:
                    ...
                    blastdbcmd_path = f"{os.path.dirname(blast_path)}/blastdbcmd"
            ...
            if blast_bin:
                if blast_bin.endswith("/bin") or blast_bin.endswith("\\bin"):
                    ...
                    blastdbcmd_path = f"{os.path.dirname(blast_bin)}/blastdbcmd"
...
class BlastResourceCard(QFrame):
    ...
    def _on_category_changed(self, index):
        ...
        path = get_database_path("blast_nt", "") if not is_custom else self.custom_input.text()
    ...
    def _on_save(self):
        ...
        db_path = get_database_path("blast_nt", "") if self.cat_combo.currentIndex() == 0 else self.custom_input.text().strip()
        if not db_path:
            return
    ...
    def get_db_path(self):
        return get_database_path("blast_nt", "") if self.cat_combo.currentIndex() == 0 else self.custom_input.text().strip()
