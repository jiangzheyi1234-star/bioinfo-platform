...
class DatabaseSettingsDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        title.setStyleSheet("font-size: 15px; font-weight: 700; color: #071828;")
        root.addWidget(title)

        section1 = QLabel("根目录路径")
        section1.setStyleSheet("font-size: 11px; color: #7EB8D0; font-weight: 600;")
    ...
    def _on_save(self) -> None:
        ok = self._save_fn(self.path_edit.text().strip())
        if ok:
            self.accept()
...
class DatabasePage(BasePage):
    ...
    def _browse_db_root(self) -> None:
        self._pick_remote_db_root(self._get_db_root() or "~")
    ...
    def _validate_db_root_remote(self, raw_path: str, allow_create: bool = False) -> tuple[bool, str, str, bool]:
            ...
            if allow_create:
                ...
                if rc_create != 0:
                    return (
                        False,
                        "",
                        f"目录不存在且无法自动创建: {resolved}\n错误: {err_create.strip() or '权限不足'}",
                        False,
                    )
                created = True
            else:
        ...
        if rc_probe != 0:
            return (
                False,
                "",
                self._build_permission_denied_message(resolved, detail=err_probe.strip() or "写入探针失败"),
                created,
            )
