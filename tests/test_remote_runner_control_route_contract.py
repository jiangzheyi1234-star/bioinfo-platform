...
def test_workflow_trigger_routes_delegate_to_service() -> None:
    ...
    assert "async def run_workflow_trigger_scheduler_once_request(" in scheduler_control_route_source
...
def test_remote_runner_main_delegates_control_plane_work_to_service() -> None:
    ...
    assert "delete_result_package_bytes_from_request" not in execution_query_route_source
    ...
    for name in (
        ...
        assert f"async def {name}(" in control_source
    assert "delete_result_package_bytes_from_request" not in control_source
