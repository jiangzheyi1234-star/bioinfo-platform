...
def test_terminal_scrollback_is_capped_on_stream_and_viewport() -> None:
    ...
    _assert_contains(
        terminal_source,
        'case "output":',
        "terminalAbsoluteCursor(item.cursor, item.base_cursor)",
        "terminalAbsoluteCursor(message.cursor, message.base_cursor)",
        "message.base_cursor > previousCursor",
        "terminalScrollbackTruncatedRef",
        "streamWasRebased",
