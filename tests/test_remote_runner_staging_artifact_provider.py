...
from tests.helpers.remote_runner_artifact_checks import add_packaged_sqlite_metadata


def _write_minimal_runner_artifact(
    path: Path, *, version: str = REMOTE_RUNNER_VERSION
) -> None:
...
def test_staged_declared_runner_bundle_requires_explicit_gate(
    ...
    bundle = (
        ...
        / "resources"
        / "remote-runner"
        / REMOTE_RUNNER_ARTIFACT.archive_filename("linux-64")
    )
    ...
    with pytest.raises(RemoteRunnerArtifactError, match="manifest sha256 mismatch"):
        RemoteRunnerArtifactProvider(repo_root=root).resolve(
            REMOTE_RUNNER_VERSION, platform="linux-64"
        )
...
def test_explicit_staged_runner_bundle_allows_unpromoted_manifest_version(
    ...
    bundle = (
        ...
        / "dist"
        / "remote-runner"
        / "h2ometa-remote-runner-0.1.4-control-plane-linux-64.tar.gz"
    )
