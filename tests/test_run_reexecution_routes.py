...
from apps.remote_runner.api_models import RunResumeRequest, RunRuleRetryRequest
...
def test_run_reexecution_requests_are_strict_and_confirmation_gated() -> None:
    ...
    with pytest.raises(ValidationError) as extra:
        RunRuleRetryRequest.model_validate(
            {"confirmation": "retry-failed-rules", "planHash": "a" * 64, "executionOptions": {}}
        )
...
def _create_failed_rule_run(cfg, run_id: str) -> None:
