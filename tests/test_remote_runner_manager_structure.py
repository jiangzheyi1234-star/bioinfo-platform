...
def test_runner_release_prune_logic_lives_in_release_prune_module() -> None:
    manager_source = _source("core/remote_runner/manager.py")
    release_prune_source = _source("core/remote_runner/release_prune.py")

    assert "from core.remote_runner.release_prune import RemoteRunnerReleasePruneMixin" in manager_source
    assert "RemoteRunnerReleasePruneMixin" in manager_source
    assert "class RemoteRunnerReleasePruneMixin" in release_prune_source
    assert "def preview_release_prune(" in release_prune_source
    assert "def run_release_prune(" in release_prune_source
    assert "H2OMETA_PRUNE_RELEASES" in release_prune_source
    assert "def preview_release_prune(" not in manager_source
    assert "def run_release_prune(" not in manager_source
