...
def test_target_acceptance_uses_unified_catalog_draft_runnable_count(
    monkeypatch,
) -> None:
    from apps.api import tool_candidate_target_acceptance
...
def test_bio_agent_catalog_target_acceptance_counts_registered_tool_contracts(
    monkeypatch,
) -> None:
    from apps.api import tool_candidate_target_acceptance
    ...
    report = tool_candidate_target_acceptance.bio_agent_catalog_target_acceptance(
        ...
        registered_tools=[
            ...
            {
                "id": "bioconda::multiqc",
                "toolContract": {"state": "ProductionEnabled", "workflowReady": True},
            },
            {
                "id": "bioconda::draft",
                "toolContract": {
    ...
    assert (
        first_item["preparePayload"]["ruleSpecDraft"]["source"]
        == "h2ometa-tool-profile"
    )
    assert (
        first_item["preparePayload"]["ruleSpecDraft"]["requiresUserCompletion"] is False
    )
    ...
    assert [item["toolId"] for item in production_queue["items"]] == [
        "bioconda::fastp",
        "bioconda::fastqc",
    ]
    ...
    assert (
        first_production_item["productionPlan"]["submit"]["pathTemplate"]
...
def test_validation_queue_prioritizes_wrapper_evidence_and_semantic_ports(
    ...
    assert len(items) >= 2
    assert all(
        items[index]["priority"]["score"] >= items[index + 1]["priority"]["score"]
...
def test_validation_queue_prioritizes_self_contained_profiles_before_required_resources(
    monkeypatch,
) -> None:
    from apps.api import tool_candidate_target_acceptance
    ...
    profile_ids = [
        str(item.get("profileId") or "") for item in report["validationQueue"]["items"]
    ]
    ...
    bcftools = next(
        ...
        if item["profileId"] == "bcftools-stats"
    )
    bedtools = next(
        item
        for item in report["validationQueue"]["items"]
        if item["profileId"] == "bedtools-bamtobed"
    )
    bowtie2 = next(
        item
        for item in report["validationQueue"]["items"]
        if item["profileId"] == "bowtie2-align"
    )
    ...
    assert (
        bedtools["preparePayload"]["ruleTemplate"]["inputs"][0]["kind"]
        == "alignment_sam"
    )
    assert (
        "samtools view -bS"
        in bedtools["preparePayload"]["ruleTemplate"]["commandTemplate"]
    )
...
def test_validation_evidence_summarizes_wrapper_contract_hints(monkeypatch) -> None:
    from apps.api import tool_candidate_target_acceptance_evidence
    from apps.api.tool_profile_sources import all_tool_profiles

    profile = next(
        profile for profile in all_tool_profiles() if profile.profile_id == "fastqc"
    )
    monkeypatch.setattr(
        tool_candidate_target_acceptance_evidence,
    ...
    evidence = tool_candidate_target_acceptance_evidence.validation_evidence(
        profile=profile,
        prepare_payload={"ruleTemplate": profile.rule_template},
    )

    assert evidence["wrapperContractHintCount"] == 1
    assert evidence["wrapperContractHintFields"] == [
        "description",
        "environment",
        "input",
        "output",
    ]
    assert evidence["wrapperCondaDependencies"] == [
        "fastqc =0.12.1",
        "snakemake-wrapper-utils =0.8.0",
    ]


def test_validation_queue_item_includes_prepare_job_validation_plan(
    monkeypatch,
) -> None:
    from apps.api import tool_candidate_target_acceptance
...
def test_validation_queue_item_exposes_execution_gate_without_marking_ready(
    monkeypatch,
) -> None:
    from apps.api import tool_candidate_target_acceptance
...
def test_validation_queue_item_includes_latest_prepare_job_without_counting_it_ready(
    monkeypatch,
) -> None:
    from apps.api import tool_candidate_target_acceptance
