...
def test_runtime_layout_records_schema_version_and_migration_ledger(tmp_path: Path) -> None:
    ...
    assert migration[1] == sqlite_migrations.WORKFLOW_TRIGGER_READINESS_WATCHER_MIGRATION_NAME
    assert migration[2]
...
def test_runtime_schema_migrates_v1_to_current_scheduler_trigger_tables(tmp_path: Path) -> None:
    ...
    assert migration["name"] == sqlite_migrations.WORKFLOW_TRIGGER_READINESS_WATCHER_MIGRATION_NAME
