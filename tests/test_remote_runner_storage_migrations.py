...
def test_runtime_layout_records_schema_version_and_migration_ledger(tmp_path: Path) -> None:
    ...
    assert migration[1] == sqlite_migrations.CURRENT_SCHEMA_MIGRATION_NAME
    assert migration[2]
...
def test_runtime_schema_migrates_v9_workflow_trigger_inbox_payload(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as connection:
        ...
        connection.execute(
            """
            CREATE INDEX idx_workflow_trigger_inbox_trigger_received
            ON workflow_trigger_inbox_events(trigger_id, received_at)
            """
        )
        connection.execute(
            """
            CREATE INDEX idx_workflow_trigger_inbox_state
            ON workflow_trigger_inbox_events(state, updated_at)
            """
        )
        connection.execute(
            """
            CREATE INDEX idx_workflow_trigger_inbox_trigger_event
        ...
        connection.execute(
            """
            INSERT INTO schema_migrations (version, name, checksum, applied_at)
            VALUES (9, '009_workflow_trigger_inbox', 'legacy-v9', '2099-06-07T10:00:00Z')
            """
        )
...
def test_runtime_schema_migrates_v12_workflow_trigger_readiness_watcher(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as connection:
        ...
        connection.executescript(f"{SCHEMA_SQL}\n{REFERENCE_DATABASE_SCHEMA_SQL}")
        sqlite_migrations._apply_baseline_schema_migration(connection)
        connection.execute("DROP INDEX IF EXISTS idx_workflow_trigger_readiness_observations_event")
        connection.execute("DROP INDEX IF EXISTS idx_workflow_trigger_readiness_observations_state")
        connection.execute("DROP TABLE IF EXISTS workflow_trigger_readiness_observations")
        sqlite_migrations._ensure_schema_migrations_table(connection)
        connection.execute("DELETE FROM schema_migrations")
        connection.execute(
            """
            INSERT INTO schema_migrations (version, name, checksum, applied_at)
            VALUES (12, '012_workflow_trigger_inbox_signature_metadata', 'legacy-v12', '2099-06-07T10:00:00Z')
            """
        )
    ...
    with get_connection(cfg) as connection:
        ...
        table = connection.execute(
            ...
            SELECT name
            FROM sqlite_master
            WHERE type = 'table' AND name = 'workflow_trigger_readiness_observations'
            """
        ).fetchone()
        index_names = {
            row["name"]
            for row in connection.execute(
                ...
                SELECT name
                FROM sqlite_master
                WHERE type = 'index' AND name LIKE 'idx_workflow_trigger_readiness_observations%'
                """
            ).fetchall()
        }
        migration = connection.execute(
...
def test_runtime_schema_migrates_v13_result_package_byte_state(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as connection:
        ...
        connection.execute(
            ...
            INSERT INTO result_package_exports (
                ...
                'rpexp_legacy_bytes', 'res_run_legacy_bytes', 'run_legacy_bytes', 'wfrev_legacy',
                'C:/packages/res_run_legacy_bytes.zip', 'file:///C:/packages/res_run_legacy_bytes.zip',
                42, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
                'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb',
                'ev_legacy', '[]', 1, 'included', 'retired', '2099-06-07T10:00:00Z'
            )
            """
        )
        connection.execute(
        ...
        connection.execute(
            """
            INSERT INTO schema_migrations (version, name, checksum, applied_at)
            VALUES (13, '013_workflow_trigger_readiness_watcher', 'legacy-v13', '2099-06-07T10:00:00Z')
            """
        )
    ...
    with get_connection(cfg) as connection:
        ...
        row = connection.execute(
            ...
            FROM result_package_exports
            WHERE package_export_id = 'rpexp_legacy_bytes'
            """
        ).fetchone()
        migration = connection.execute(
            "SELECT name FROM schema_migrations WHERE version = ?",
    ...
    assert {"package_bytes_state", "package_bytes_deleted_at", "package_bytes_gc_reason"} <= columns
    assert dict(row) == {
