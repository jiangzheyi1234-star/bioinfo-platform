...
def test_result_package_file_io_lives_in_remote_service_not_routes() -> None:
    ...
    endpoint_caller_source = _source("core/remote_runner/endpoint_caller.py")
    ...
    assert "delete_result_package_bytes_from_request" not in route_source
    assert "/bytes/delete" not in route_source
    ...
    assert not (ROOT / "core/remote_runner/result_package_proxy.py").exists()
    assert "RemoteRunnerResultPackageProxyMixin" not in manager_source
    assert 'client.get_json(f"/api/v1/results/{kwargs[\'result_id\']}/audit")["data"]' not in proxy_source
    assert 'endpoint.response_transport == "bytes"' in endpoint_caller_source
    assert "return client.download_bytes(path, **status_kwargs)" in endpoint_caller_source
    assert "client.download_result_package(" not in proxy_source
    assert "client.list_result_package_exports(" not in proxy_source
    assert "client.retire_result_package(" not in proxy_source
    assert "client.preview_result_package_byte_gc(" not in proxy_source
    assert "client.run_result_package_byte_gc(" not in proxy_source
    assert "client.delete_result_package_bytes(" not in proxy_source
    ...
    assert "def download_bytes(" in client_source
    assert "path: str" in client_source
    ...
    assert '"response_transport": "bytes"' in result_package_contracts_source
    assert (
