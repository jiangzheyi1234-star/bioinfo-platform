...
def test_workspace_opens_directly_without_a_first_run_wizard() -> None:
    ...
    assert 'redirect("/workflows")' in root_page
    assert not FIRST_RUN_ROUTE.exists()
    assert "/workflows/first-run" not in tabs
    assert "/workflows/first-run" not in ssh_shell_ui
    assert "首跑向导" not in ssh_shell_ui
