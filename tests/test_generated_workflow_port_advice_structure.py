...
def test_canvas_converter_advice_supports_safe_auto_insertion() -> None:
    ...
    assert "export function automaticConverterInsertionRequestForConnection" in port_advice_contract
    assert 'insertionMode: "automatic-unambiguous"' in port_advice_contract
    assert "targetInputAlreadyBound" in port_advice_contract
    ...
    assert "automaticConverterInsertionRequestForConnection" in graph_canvas_ui
    ...
    assert "已自动插入转换节点" in graph_canvas_ui
    ...
    assert "onInsertConverter(notice.autoInsertionRequest)" in graph_canvas_ui
    assert graph_canvas_ui.count("onInsertConverter(connectionNotice.request)") == 1
    assert graph_canvas_ui.count("onInsertConverter(notice.autoInsertionRequest)") == 1
    ...
    assert "onInsertConverter(notice.autoInsertionRequest)" in on_connect_end_body
