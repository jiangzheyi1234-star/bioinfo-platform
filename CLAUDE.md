...
| 文件 | 状态 |
|------|------|
| `ui/pages/project_page.py` | ✅ 项目创建/切换/归档/导出 |
...
| `ui/pages/detection_page.py` | ⚠️ blastn，待合并至读长分析页（作为病原体标注结果内嵌） |
| `ui/pages/settings_page.py` | ✅ SSH/NCBI 配置 |
| `ui/pages/home_page.py` | ⚠️ 旧架构（NCBI 检索 + 数据库构建），未迁移 |
...
pytest，`bio_ui` conda 环境（Python 3.11），`tests/` 目录，18 个测试文件
