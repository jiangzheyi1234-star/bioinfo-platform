...
from core.remote_runner.remote_runner_artifact_validation import (
    REQUIRED_WRAPPER_ASSET_MEMBERS,
)
from core.remote_runner.remote_runner_artifact_validation import (
...
from core.remote_runner.remote_runner_artifact_validation import verify_required_wrapper_assets  # fmt: skip  # noqa: E501
from core.remote_runner.remote_runner_artifact_resolution import (
...
class RemoteRunnerArtifactProvider:
    ...
    def resolve(
        self, version: str = REMOTE_RUNNER_VERSION, *, platform: str = "linux-64"
    ) -> RemoteRunnerArtifact:
        ...
        if is_declared_release_artifact(
            ...
            repo_root=self._repo_root,
        ) and not explicit_staging_runner_bundle_allowed(archive_path):
            verify_declared_artifact_metadata(
                REMOTE_RUNNER_ARTIFACT,
                platform=platform,
                archive_path=archive_path,
        ...
        staging_runner_bundle_allowed = explicit_staging_runner_bundle_allowed(
            archive_path
        )
        manifest = read_remote_runner_bootstrap_manifest(archive_path)
        if str(manifest.get("service") or "") != REMOTE_RUNNER_ARTIFACT.service:
            raise RemoteRunnerArtifactError(
                f"remote runner artifact manifest has unexpected service: {archive_path}"
            )
        manifest_version = validated_remote_runner_manifest_version(
            manifest, archive_path
        )
        if manifest_version != version and not staging_runner_bundle_allowed:
            raise RemoteRunnerArtifactError(
                f"remote runner artifact manifest version mismatch: {archive_path}"
            )
        if str(manifest.get("platform") or "") != platform:
            raise RemoteRunnerArtifactError(
                f"remote runner artifact manifest platform mismatch: {archive_path}"
            )
...
class WorkflowRuntimeArtifactProvider:
    ...
    def resolve(
        self, version: str = WORKFLOW_RUNTIME_VERSION, *, platform: str = "linux-64"
    ) -> WorkflowRuntimeArtifact:
        archive_path = resolve_archive_path(
            WORKFLOW_RUNTIME_ARTIFACT,
            version=version,
        ...
        if not checksum_path.exists():
            raise RemoteRunnerArtifactError(
                f"workflow runtime artifact checksum not found: {checksum_path}"
            )
        expected = read_expected_sha256(checksum_path)
        actual = sha256_file(archive_path)
        if actual != expected:
            raise RemoteRunnerArtifactError(
                f"workflow runtime artifact sha256 mismatch: {archive_path}"
            )
        if is_declared_release_artifact(
            WORKFLOW_RUNTIME_ARTIFACT,
            version=version,
        ...
        if str(manifest.get("service") or "") != WORKFLOW_RUNTIME_ARTIFACT.service:
            raise RemoteRunnerArtifactError(
                f"workflow runtime artifact manifest has unexpected service: {archive_path}"
            )
        if str(manifest.get("version") or "") != version:
            raise RemoteRunnerArtifactError(
                f"workflow runtime artifact manifest version mismatch: {archive_path}"
            )
        if str(manifest.get("platform") or "") != platform:
            raise RemoteRunnerArtifactError(
                f"workflow runtime artifact manifest platform mismatch: {archive_path}"
            )
        if str(manifest.get("provider") or "") != "conda-pack":
            raise RemoteRunnerArtifactError(
                f"workflow runtime artifact must declare conda-pack provider: {archive_path}"
            )
        packages = (
        ...
        if not snakemake_package:
            raise RemoteRunnerArtifactError(
                f"workflow runtime artifact must declare snakemake package version: {archive_path}"
            )
        entrypoints = (
        ...
        snakemake_entrypoint = str(
            entrypoints.get("snakemake") or "workflow-env/bin/snakemake"
        )
        conda_unpack_entrypoint = str(
            entrypoints.get("condaUnpack") or "workflow-env/bin/conda-unpack"
        )
        ...
        for label, value in (
            ...
            if value.startswith("/") or ".." in Path(value).parts:
                raise RemoteRunnerArtifactError(
                    f"workflow runtime artifact has invalid {label} entrypoint: {archive_path}"
                )
        verify_workflow_runtime_contents(
            archive_path,
