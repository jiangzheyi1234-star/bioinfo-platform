...
class TaskRecoveryWorker(QThread):
    ...
    all_checked = pyqtSignal(int, int, int)
    progress = pyqtSignal(str)

    def __init__(self, client_provider, tasks_to_check: list = None):
        super().__init__()
        self.client_provider = client_provider
        self.tasks_to_check = tasks_to_check
        self.local_out_dir = str(get_runtime_setting("local_output_dir", "") or "")
