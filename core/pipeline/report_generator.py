...
class ReportGenerator:
    ...
    def _page_methods(plt, pdf, analysis_method, summary):
        ...
        lines = [
            ...
            "",
            "软件与数据库：",
            "  - Kraken2 / Centrifuge：基于 k-mer 的快速物种分类",
            "  - BLASTn：核苷酸序列比对（NCBI core_nt 数据库）",
            "  - fastp：测序数据质控与过滤",
            "  - hostile：人类宿主序列去除",
            "  - MEGAHIT：宏基因组 de novo 组装",
            "",
            "流程说明：",
            "  1. 原始测序数据经 fastp 质控后，通过 hostile 去除宿主序列",
            "  2. 清洁 reads 使用 Kraken2/Centrifuge 进行快速物种分类",
            "  3. 未分类 reads 经 MEGAHIT 组装为 contigs",
            "  4. Contigs 通过 BLASTn 比对 core_nt 数据库补充鉴定",
            "  5. 合并两步结果，输出完整物种组成表",
            "",
