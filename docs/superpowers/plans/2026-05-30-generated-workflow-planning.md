...
- [ ] Add a focused test proving preflight accepts the same implicit single-tool payload shape that rendering uses, including top-level params.
...
- [ ] Keep generated graph normalization aligned with the shared planner for `toolId`, named uploaded inputs, and exposed output aliases.
