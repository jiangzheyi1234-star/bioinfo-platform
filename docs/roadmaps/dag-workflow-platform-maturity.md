...
- Artifact lifecycle controller ticks now record a durable preview-only policy decision, retention-hold summary, and batch-safety summary. The controller evidence explains quota/TTL candidates, protected hold reasons, and `maxDeleteBytesPerTick` limiting without storing raw paths, storage URIs, group IDs, or executing deletion.
...
- Artifact GC preview/run routes now return public projections for operator/API responses. Internal GC plans and evidence keep deletion locators for auditability, but external payloads expose only plan ids, policy, counts, bytes, backend labels, retention reasons, and payload deletion booleans; storage URIs, local paths, group ids, artifact ids, run ids, materialization ids, and SHA-256 values stay internal.
...
- Artifact GC preview/run responses now use public lifecycle projections while the executor and evidence ledger retain raw deletion locators internally. Failed delete responses return stable error codes instead of forwarding path-, object-, or digest-bearing exception strings to API clients.
