...
- Rule retry executor orchestration now includes a read-only `rule-partial-rerun-execution-boundary.v1` contract. It makes the remaining Snakemake target/finalization risk explicit: current rule-rerun commands have no explicit output targets in a fresh attempt work/result directory, and successful candidate-output adoption would finalize the whole run. Public rule-retry mutation remains blocked until explicit targets and finalize boundaries are proven.
