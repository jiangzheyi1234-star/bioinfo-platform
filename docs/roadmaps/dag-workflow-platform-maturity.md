...
- Result artifact preview now fails closed before reading payload content unless the selected active artifact has complete managed metadata and its live size/SHA-256 matches the stored record; corrupted local files and corrupted S3/MinIO directory packages are rejected with explicit checksum-audit errors.
