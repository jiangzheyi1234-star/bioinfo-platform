...
- Cache lookup is traceable through `artifact.cache.lookup.v1` evidence. After a successful dry-run, the worker can now adopt a full set of cache-hit output artifacts into the current attempt, restore the cached payload to each declared output path, write `artifact.cache.adopt.v1` evidence, record a local materialization, mark rules as cache-hit succeeded, and skip the expensive Snakemake run. Per-rule partial restore, downstream invalidation, cache pinning, and broader staged-file policy controls remain pending.
...
6. Extend full-output cache adoption into per-rule restore only after per-rule cache eligibility, cache pinning, downstream invalidation, and staged-file policy controls are represented in run events.
