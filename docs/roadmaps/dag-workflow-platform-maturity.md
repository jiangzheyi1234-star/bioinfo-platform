...
Baseline: `codex/dag-workflow-maturity-phase-2`, `HEAD=49af26110d07593b7116222216740b6231cded40`.
...
- Rule retry executor orchestration now includes a read-only `rule-partial-rerun-execution-boundary.v1` contract. The Snakemake target risk is reduced through a key-based `targetOutputKeys` contract: execution plans, launch preflight, worker claim preflight, and executor options carry only output keys, while the executor maps those keys to managed Snakemake positional file targets immediately before dry-run/run. Public rule-retry mutation remains blocked because successful candidate-output adoption would still finalize the whole run until the finalize boundary is proven.
