...
- Per-rule staged restore now consumes those active attempt-owned restore pins through a separate prepare/apply contract. Apply restores cache payloads to a managed attempt staging area and records internal evidence/materialization ids while public responses and governance audit expose only counts and booleans. Final-output overwrite, staged-to-final promotion, restore-pin release orchestration, and Snakemake partial retry execution remain pending.
