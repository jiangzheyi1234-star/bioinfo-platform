...
- Result artifact preview now fails closed before reading payload content unless the selected active artifact has complete managed metadata, points at a managed local result/work path or configured S3/MinIO artifact prefix, and its live size/SHA-256 matches the stored record; corrupted local files, unmanaged paths/objects, and corrupted S3/MinIO directory packages are rejected with explicit audit/storage errors.
