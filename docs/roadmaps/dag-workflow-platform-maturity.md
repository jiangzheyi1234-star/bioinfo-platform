...
- Manual Runner Stop is now a sticky runtime intent across the local control plane. Reconnect, startup auto-connect, service-info, existing-runner diagnostics/proxy reads, upgrade, and token rotation no longer touch or restart the remote runner while the saved health snapshot carries `RUNNER_STOPPED`; the explicit Runner Start action is the only lifecycle path that clears the stopped state.
