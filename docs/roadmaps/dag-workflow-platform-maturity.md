...
- Per-rule staged restore now consumes those active attempt-owned restore pins through separate staged-file and final-output promotion contracts. Staged apply restores cache payloads to a managed attempt staging area; final-output apply promotes them only into declared current-attempt output paths and records pending candidate outputs. Public responses and governance audit expose only counts and booleans. Artifact adoption, restore-pin release orchestration, and Snakemake partial retry execution remain pending.
