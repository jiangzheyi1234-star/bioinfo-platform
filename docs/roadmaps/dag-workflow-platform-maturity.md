...
- Rule cache restore preflight now includes a path-redacted restore-pin policy preview. Cache-hit outputs can be counted as future attempt-scoped restore-pin candidates only after output invalidation is applied, but pin creation remains disabled, no restore pins are written, and run execution stays blocked until active-lease fencing, pin creation/release, staged file overwrite, and partial restore executor contracts are proven.
...
- Per-rule restore planning now represents restore-pin readiness as a preview-only, path-redacted policy contract. It reports candidate/required/eligible/blocked/created pin counts and the future restore pin scope/owner kind/TTL while keeping owner ids, cache keys, storage URIs, paths, actual pin creation, lookup evidence, and cache hit count mutation out of the read model.
