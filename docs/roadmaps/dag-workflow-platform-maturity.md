...
- The web UI now also has a read-only trigger/event observability surface under the run results area. It lists trigger definitions, source-aware trigger specs, immutable events, payload hashes, safe event-context/resource/schedule summaries, dispatch state, request ids, and run links while omitting raw payloads, runSpec JSON, create/enable/disable/pause/resume/replay/catchup/concurrency controls, and other unsupported scheduler operations.
