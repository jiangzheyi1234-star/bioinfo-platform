...
- Result package byte-GC preview is now a governed high-risk remote action (`result.package.bytes.preview`) with artifact-curator/auditor role coverage. It remains read-only, separates result-package ZIP byte lifecycle from artifact payload GC, and returns only a public aggregate projection with candidate/protected reason counts and a plan fingerprint.
