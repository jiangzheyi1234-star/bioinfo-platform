...
- Rule retry execution context now includes a read-only `rule-partial-rerun-output-closure.v1` contract. It cross-checks scoped restored-output adoption, preserved-rule active output edges, unknown active outputs, incomplete-output audit safety, and path/storage redaction before executor orchestration can become contract-ready; full finalize remains blocked until declared output closure is proven.
