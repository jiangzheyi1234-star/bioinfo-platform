...
- The trigger observability UI now has a constrained trigger-definition create surface for `manual`, `cron`, and `backfill` definitions. It builds canonical payloads from explicit fields, restricts cron overlap policy to `Forbid`/`Allow`, keeps webhook/readiness creation API-only until signed delivery and watcher setup have their own controls, and avoids an advanced JSON fallback.
- Manual trigger definitions now have a confirmation-gated UI action that submits exactly one manual trigger event through the existing immutable trigger event, run admission, run creation, and provenance path. Cron catchup, webhook generic dispatch, readiness push, backfill launch, and pause/resume controls remain outside this observability surface.
...
- Manual Runner Stop is now a sticky runtime intent across the local control plane. Stop writes an explicit `runner_stop_intent.active=true` record, reconnect, startup auto-connect, service-info, existing-runner diagnostics/proxy reads, upgrade, and token rotation no longer touch or restart the remote runner while that intent is active, and the explicit Runner Start action is the only lifecycle path that clears it. Older snapshot-only `RUNNER_STOPPED` records are treated as unsupported lifecycle state (`RUNNER_STOP_INTENT_REQUIRED`) and require explicit Start rather than silent auto-start.
- Remote runner upgrade/bootstrap now uses the same execution-idle contract as prune and uninstall before any destructive bundle deploy. The guard blocks not only active workflow leases but also allocated resources, queued resource waits, claimed jobs, running worker slots, and unhealthy execution diagnostics, while preserving the existing active-lease reason code for product surfaces.
...
- Remote runner operator diagnostics now include user-systemd linger status and redacted `journalctl --user` tail evidence alongside the existing current-release, artifact-marker, runtime-state, systemd status, and runner.log probes. The lifecycle bundle keeps remote paths hashed/redacted and redacts token/password/secret/API-key log material before it reaches the local API.
