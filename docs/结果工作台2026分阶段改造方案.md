...
### Phase 1 当前实施进度（2026-03-31，已完成）

已落地并完成验收：

- `ui/pages/detection_page_assets/app_galaxy.js` 已新增统一 execution 导航入口 `openExecution(executionId, context)`
- `onRunResult()` 已接入 `execution_id` 自动定位，不再只做提示与刷新
- history 中“查看结果”“查看状态”已统一先走 `openExecution()`
- 当前路由规则已按本 phase 生效：
  - `pending / running / failed` -> history 状态详情
  - `completed` -> `get_results_for_execution()` -> integrated 结果壳
- 自动聚焦 history execution 时会清空现有搜索关键字，避免新 execution 被筛选条件隐藏
- 已补齐本 phase 的最小回归与 gate 护栏，覆盖：
  - `onRunResult()` / execution 路由主干资产锁定
  - history 空态展示与 history action 显隐契约
  - `sample_name` 缺失时 history / result 构建仍稳定回退
  - 无项目时 `get_execution_history()` 返回空列表
  - 无项目时 `get_results_for_execution()` 返回稳定 error payload
  - SSH 未连接时远端状态接口保留本地状态，不因 `pending` 误报失败

本 phase gate 结论：

- `运行后自动定位对应 execution_id`：通过
- `history 与运行成功入口共用同一路由`：通过
- `空 history / 无项目 / 无样本 / SSH 未连接时不崩溃`：通过

本次明确未做：

- 未重构 integrated 页面职责
- 未引入 typed result UX
- 未修改结果 payload / archetype / backend builder
- 未修改 `ToolEngine.execute()`、SSH 链路、execution 状态枚举

后续进入 Phase 2 时应以此为当前代码基线，禁止重复引入平行 execution 路由入口。
...
### Phase 2 当前实施进度（2026-03-31，已完成）

已落地并完成验收：

- `ui/pages/detection_page_assets/index_galaxy.html` 与 `ui/pages/detection_page_assets/app_galaxy.js` 已在保留原 DOM 宏布局的前提下，将 integrated 前端行为收口为统一结果壳
- `Overview / Result / Files / Provenance` 二级结构已固定存在，结果工作台 tab 已具备切换行为
- 当前固定映射已按本 phase 生效：
  - `Overview` -> summary + sections
  - `Result` -> html/chart/table
  - `Files` -> artifacts
  - `Provenance` -> 参数、版本、目录、execution_id
- 从 history 进入 completed execution 时，workflow run entry 已隐藏，仅展示统一结果壳
- 从 workflow 入口进入时，仍保留 run entry，不影响既有提交入口
- history result 与 workflow baseline view 已在前端内存态分离，history 结果不会覆盖默认工作台基线视图
- integrated history result shell 缓存已按 `project_id` 作用域隔离；项目切换时会清理旧项目 execution 结果壳，避免跨项目污染
- 结果空态文案已去除“右侧执行入口”导向，改为统一结果壳语义
- 已补齐本 phase 的资产级 smoke 护栏，覆盖：
  - 四段结果壳结构与 tab 绑定存在性
  - history 上下文与 workflow 上下文分离契约
  - history completed result 隐藏 run entry 的行为痕迹
  - 结果空态文案不再回退到 workflow 导向
- 本次定向测试已通过：
...
- `integrated 不再默认同时承担 workflow 启动和历史结果落点`：通过
- `Overview / Result / Files / Provenance 固定存在`：通过
- `completed execution 从 history 进入时只看到结果壳`：通过

本次明确未做：

- 未改结果 payload / archetype / backend builder / bridge API
- 未引入 typed result UX
- 未做大规模 CSS 美化或视觉重设
- 未修改 `ToolEngine.execute()`、SSH 链路、execution 状态枚举、持久化状态
