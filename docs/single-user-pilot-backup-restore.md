...
The proof must report `closedLoopProven: true`, `closedLoopProofMode: "submitted-run"`, `executionReadinessProof.ok: true`, `sampleUploadProof.schemaVersion: "h2ometa.first-run.sample-upload-proof.v1"`, `sampleUploadProof.passed: true`, `sampleUploadProof.unexpectedRoles: []`, `sampleUploadProof.duplicateRoles: []`, a ready validation card, a result package SHA-256, and a `handoffProof` summary whose `pilotHandoff` evidence matches the validation card and result package hashes. The same handoff proof must include the read-only backup plan command, the restore proof command, and the next taxonomy/AMR scenario pilots.
