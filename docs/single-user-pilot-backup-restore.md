...
The proof must report `closedLoopProven: true`, `closedLoopProofMode: "submitted-run"`, `runTimingProof.schemaVersion: "h2ometa.first-run.timing-proof.v1"`, `runTimingProof.completedWithinTimeout: true`, `runTimingProof.withinExpectedDurationWindow: true`, `executionReadinessProof.ok: true`, `sampleUploadProof.schemaVersion: "h2ometa.first-run.sample-upload-proof.v1"`, `sampleUploadProof.passed: true`, `sampleUploadProof.unexpectedRoles: []`, `sampleUploadProof.duplicateRoles: []`, a ready validation card, a persisted completion proof, a result package SHA-256, a ready first-run evidence bundle, and a `handoffProof` summary whose `completionProof` and `pilotHandoff` evidence match the validation card and result package hashes. The same handoff proof must report `handoffProof.resultId`, `handoffProof.workflowRevisionId`, `handoffProof.packageExportId`, `handoffProof.reportEvidence`, `handoffProof.validationCard.validationCardJsonSha256`, `handoffProof.completionProof.savedAt`, `handoffProof.completionProof.validationCardJsonSha256`, `handoffProof.completionProof.evidenceBundleZipFileRoles`, `handoffProof.resultPackageDownload`, `handoffProof.evidenceBundleSchemaVersion: "h2ometa.first-run.evidence-bundle.v1"`, `handoffProof.evidenceBundleFileRoles: ["result-package", "validation-card-json", "validation-card-markdown", "pilot-handoff"]`, `handoffProof.evidenceBundleDownload` for the portable ZIP, `handoffProof.evidenceBundleDownload.zipManifestSha256`, `handoffProof.evidenceBundleDownload.completionProofJsonSha256`, `handoffProof.evidenceBundleDownload.validationCardJsonSha256`, the read-only backup plan command, the restore proof command, and `handoffProof.nextScenarioDatabasePackCoverage` for the blocked taxonomy/AMR database pilot gates. Passing `-ProofPath` stores the same machine-readable summary as a durable pilot evidence file.
