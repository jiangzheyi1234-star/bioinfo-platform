...
- Result package byte-GC preview (`result.package.bytes.preview`) requires artifact-curator/auditor roles and is read-only. It uses durable result package export metadata, authoritative `retired_at`, retention policy, optional byte caps, and live managed-package size/SHA-256 verification to classify candidates and protected records while returning only public counts, bytes, reason codes, redaction policy, and a plan fingerprint. Raw result IDs, run IDs, export IDs, package paths, storage URIs, and SHA-256 values remain internal.
