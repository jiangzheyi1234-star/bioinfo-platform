...
- Run observability reads (`run.events.read`, `run.execution_context.read`, `run.attempts.read`, `run.logs.read`, `run.rules.read`, and `run.failure_locator.read`) require workflow-operator/auditor roles before run event, execution context, attempt, log, rule, or failure-locator storage is read. The rule and failure-locator public read models expose safe rule identity, attempt/lease, status, timings, counts, aggregate rule/log-evidence summaries, capped tails, managed artifact summaries, and explicit redaction policy while excluding rule input/output paths, log paths, artifact paths, storage URIs, command summaries, run specs, and raw sensitive event details. Successful reads write hash-chained allow audit summaries with counts, state distributions, stream labels, cursor-presence booleans, rule-log evidence reason/status counters, and failure-locator reason/status counters, while excluding log lines, rule-log tails, failure tails, event detail payloads, run specs, command summaries, command args, local paths, storage URIs, artifact paths, and raw cursor values.
