...
- Artifact GC preview/run routes require artifact-curator roles and return only public lifecycle projections with plan ids, policy, counts, bytes, backend labels, and retention reasons. Storage URIs, local paths, lifecycle group ids, artifact ids, run ids, materialization ids, and SHA-256 values remain internal to the deletion executor and durable evidence ledger.
