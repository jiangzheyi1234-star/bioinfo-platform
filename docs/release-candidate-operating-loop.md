...
The restore is not accepted until that command reports `closedLoopProven: true`, `closedLoopProofMode: "submitted-run"`, `executionReadinessProof.ok: true`, `sampleUploadProof.schemaVersion: "h2ometa.first-run.sample-upload-proof.v1"`, `sampleUploadProof.passed: true`, `sampleUploadProof.unexpectedRoles: []`, `sampleUploadProof.duplicateRoles: []`, a ready validation card, a result package SHA-256, and a `handoffProof` summary whose `pilotHandoff` evidence matches the validation card and result package hashes.
