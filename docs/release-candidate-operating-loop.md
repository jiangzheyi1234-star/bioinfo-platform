...
This verifies that the Moving Pictures 16S reference workflow is runnable, the scenario pack points at its standard workflow detail page, the required result evidence is advertised, and the normal workflow workspace is served. Without `-RunId` or `-RunFirstSuccessfulRun`, the JSON summary must report `closedLoopProven: false` with `closedLoopProofMode: "catalog-page-smoke"`; that is a smoke proof, not a completed reference run. After an operator completes a real Moving Pictures run, pass `-RunId <run_id>` to call the finalization API and require either a ready validation card/result package/evidence bundle or a typed blocked `nextAction` that targets the standard workflow or results workspace. Add `-RequireFinalizationReady` with `-RunId` or `-RunFirstSuccessfulRun` when the pilot handoff must fail unless the result package, validation card, and evidence bundle are complete.
...
The restore is not accepted until that command reports `closedLoopProven: true`, `closedLoopProofMode: "submitted-run"`, `runTimingProof.schemaVersion: "h2ometa.first-run.timing-proof.v1"`, `runTimingProof.completedWithinTimeout: true`, `runTimingProof.withinExpectedDurationWindow: true`, `executionReadinessProof.ok: true`, `sampleUploadProof.schemaVersion: "h2ometa.first-run.sample-upload-proof.v1"`, `sampleUploadProof.passed: true`, `sampleUploadProof.unexpectedRoles: []`, `sampleUploadProof.duplicateRoles: []`, a ready validation card, a result package SHA-256, a ready first-run evidence bundle, and a `handoffProof` summary whose `pilotHandoff` evidence matches the validation card and result package hashes. The restore handoff must also report `handoffProof.resultId`, `handoffProof.workflowRevisionId`, `handoffProof.packageExportId`, `handoffProof.reportEvidence`, `handoffProof.validationCard.validationCardJsonSha256`, `handoffProof.resultPackageDownload`, `handoffProof.evidenceBundleSchemaVersion`, the four evidence bundle file roles, `handoffProof.evidenceBundleDownload` for the portable ZIP, `handoffProof.evidenceBundleDownload.zipManifestSha256`, `handoffProof.evidenceBundleDownload.validationCardJsonSha256`, the read-only backup/restore commands, and `handoffProof.nextScenarioDatabasePackCoverage` for the taxonomy/AMR database pilot gates.
