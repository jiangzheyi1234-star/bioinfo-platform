...
- `first-run-pilot-proof.json` and `single-user-pilot-backup-plan.json` when local single-user pilot proof is requested

The JSON summary must record the source commit, branch, generated timestamp, script path, CI run URL, Security Analysis evidence mode, Container Image Scan evidence mode, API/Web bases, launcher dev-cache root, first-run pilot proof path, single-user backup plan path, handoff eligibility, local single-user proof eligibility, gate results, skipped optional gates, and known scoped limits. Development-only proof may omit the CI URL and platform evidence, but it must report `handoffEligible: false`.
...
This command launches `run.bat --web` headlessly, waits for API `/health` and the Web root to return OK, runs `scripts/local_web_smoke.ps1`, executes Playwright through `npm run test:e2e` for the requested repeat count, and records `first-run-pilot-proof.json` through `scripts/first_run_pilot_check.ps1`. It proves the UI plus first successful run path with `firstRunPilotProof.closedLoopProven: true`, `firstRunPilotProof.runTimingProof.withinExpectedDurationWindow: true`, and `firstRunPilotCompletionProofAccepted: true`, but the summary reports `localSingleUserProofEligible: true` only after the backup plan gate below is also passed. Fresh `submitted-run` proof and reused `finalized-run` proof both count only after the same ready finalization, persisted completion-proof, validation-card hash, result-package hash, and evidence-bundle checks pass.

For a lab pilot handoff, also pass the remote runner shared state root so the same RC evidence loop proves the read-only backup plan:

```powershell
scripts\verify_release_candidate.ps1 -DevelopmentOnly -StartLocalWeb -UseUserAppStateForLocalWeb -RunWebE2E -WebE2ERepeat 3 -RunFirstRunPilotProof -RunSingleUserPilotBackupPlan -SingleUserPilotRemoteRunnerSharedRoot "/home/<user>/.h2ometa/runner/shared"
```

`localSingleUserProofEligible` stays false until `singleUserPilotBackupPlan.readyForManualBackup: true`, `singleUserPilotBackupPlan.firstRunProof.accepted: true`, and `singleUserPilotBackupPlan.firstRunProof.summary.runTimingProof.withinExpectedDurationWindow: true` are recorded in `single-user-pilot-backup-plan.json`.
The backup plan gate requires a fresh `submitted-run` proof from `-RunFirstRunPilotProof`; it fails closed when the pilot proof is reused through `-FirstRunPilotRunId`.
...
This verifies that the Moving Pictures 16S first-run pipeline is runnable, the scenario pack points at `/workflows/first-run`, the required result evidence is advertised, and the first-run page bundle is served. Without `-RunId` or `-RunFirstSuccessfulRun`, the JSON summary must report `closedLoopProven: false` with `closedLoopProofMode: "catalog-page-smoke"`; that is a smoke proof, not a completed first-run proof. After an operator completes a real Moving Pictures run, pass `-RunId <run_id>` to call the first-run finalization API and require either a ready validation card/result package/evidence bundle/completion proof or a typed blocked `nextAction` that targets an existing first-run recovery anchor. Add `-RequireFinalizationReady` with `-RunId` or `-RunFirstSuccessfulRun` when the pilot handoff must fail unless the result package, validation card, evidence bundle, and persisted completion proof are complete.

To let the pilot proof drive the whole Moving Pictures path through the same public API used by the UI, run:

```powershell
scripts\first_run_pilot_check.ps1 -RunFirstSuccessfulRun -RequireFinalizationReady -ProofPath ".\release-evidence\first-run-pilot-proof.json"
```

This selects a ready server, proves `/execution-diagnostics` readiness before submission, prepares the official Moving Pictures sample uploads with checksum verification, submits `/api/v1/runs`, polls `/api/v1/runs/<run_id>/detail` until completion, records `runTimingProof.schemaVersion: "h2ometa.first-run.timing-proof.v1"`, `runTimingProof.completedWithinTimeout: true`, and `runTimingProof.withinExpectedDurationWindow: true` with a timeout budget no greater than 1800 seconds, then finalizes the first run. It must report `closedLoopProven: true`, `closedLoopProofMode: "submitted-run"`, `executionReadinessProof.ok: true`, `sampleUploadProof.schemaVersion: "h2ometa.first-run.sample-upload-proof.v1"`, `sampleUploadProof.passed: true`, `sampleUploadProof.unexpectedRoles: []`, and `sampleUploadProof.duplicateRoles: []`; otherwise the pilot has not proven the full first successful run. Ready finalization must also prove that persisted `completionProof` matches the validation card, result package, downloaded validation-card hash, evidence bundle roles, and portable evidence ZIP roles, that `pilotHandoff` evidence matches the validation card and result package hashes, downloads the full result package and verifies its SHA-256, exposes a ready first-run evidence bundle listing the result package, validation card JSON/Markdown, and pilot handoff files, downloads the portable evidence ZIP, verifies its `MANIFEST.json` with `MANIFEST.sha256`, verifies the ZIP's saved completion-proof JSON, includes the read-only backup/restore handoff commands, and advertises the next taxonomy/AMR scenario pilots. The JSON summary records these fields under `executionReadinessProof`, `sampleUploadProof`, `runTimingProof`, and `handoffProof`, including `handoffProof.resultId`, `handoffProof.workflowRevisionId`, `handoffProof.packageExportId`, `handoffProof.reportEvidence`, `handoffProof.validationCard.validationCardJsonSha256`, `handoffProof.completionProof.savedAt`, `handoffProof.completionProof.validationCardJsonSha256`, `handoffProof.completionProof.evidenceBundleZipFileRoles`, `handoffProof.resultPackageDownload`, `handoffProof.evidenceBundleFileRoles`, `handoffProof.evidenceBundleDownload`, `handoffProof.evidenceBundleDownload.zipManifestSha256`, `handoffProof.evidenceBundleDownload.completionProofJsonSha256`, `handoffProof.evidenceBundleDownload.validationCardJsonSha256`, and `handoffProof.nextScenarioDatabasePackCoverage`. Passing `-ProofPath` writes the same machine-readable summary to disk for handoff review.
...
```powershell
scripts\first_run_pilot_check.ps1 -RunFirstSuccessfulRun -RequireFinalizationReady -ProofPath ".\release-evidence\restore-first-run-proof.json"
```

The restore is not accepted until that command reports `closedLoopProven: true`, `closedLoopProofMode: "submitted-run"`, `runTimingProof.schemaVersion: "h2ometa.first-run.timing-proof.v1"`, `runTimingProof.completedWithinTimeout: true`, `runTimingProof.withinExpectedDurationWindow: true`, `executionReadinessProof.ok: true`, `sampleUploadProof.schemaVersion: "h2ometa.first-run.sample-upload-proof.v1"`, `sampleUploadProof.passed: true`, `sampleUploadProof.unexpectedRoles: []`, `sampleUploadProof.duplicateRoles: []`, a ready validation card, a persisted completion proof, a result package SHA-256, a ready first-run evidence bundle, and a `handoffProof` summary whose `completionProof` and `pilotHandoff` evidence match the validation card and result package hashes. The restore handoff must also report `handoffProof.resultId`, `handoffProof.workflowRevisionId`, `handoffProof.packageExportId`, `handoffProof.reportEvidence`, `handoffProof.validationCard.validationCardJsonSha256`, `handoffProof.completionProof.savedAt`, `handoffProof.completionProof.validationCardJsonSha256`, `handoffProof.completionProof.evidenceBundleZipFileRoles`, `handoffProof.resultPackageDownload`, `handoffProof.evidenceBundleSchemaVersion`, the four evidence bundle file roles, `handoffProof.evidenceBundleDownload` for the portable ZIP, `handoffProof.evidenceBundleDownload.zipManifestSha256`, `handoffProof.evidenceBundleDownload.completionProofJsonSha256`, `handoffProof.evidenceBundleDownload.validationCardJsonSha256`, the read-only backup/restore commands, and `handoffProof.nextScenarioDatabasePackCoverage` for the taxonomy/AMR database pilot gates.
...
- Single-user pilot backup plan: pass `-RunSingleUserPilotBackupPlan -SingleUserPilotRemoteRunnerSharedRoot "<remote-shared-root>"` after first-run proof to write `single-user-pilot-backup-plan.json` and require `readyForManualBackup: true`.
...
If an optional gate is skipped, the JSON summary must say so. Skipping the First Successful Run pilot proof or the single-user pilot backup plan blocks `localSingleUserProofEligible`; skipping runtime release gates blocks claiming runtime artifact production readiness.
