...
Tool search, workflow step selection, recommendation, and validation queue UI share `CapabilityGraphSnapshot` as their only read model. The snapshot exposes catalog projections, semantic graph nodes, registered tools, `agentSelectableTools`, selection policy, target acceptance, and prepare queues in one contract. The old `/candidates`, `/candidate-recommendations`, and `/target-acceptance` read endpoints are removed; frontend and agent-facing selection flows must consume `/api/v1/tool-capabilities/capability-graph` and only add workflow steps from snapshot tools whose contract state is `WorkflowReady` or `ProductionEnabled` and whose `toolRevisionId` is present.
