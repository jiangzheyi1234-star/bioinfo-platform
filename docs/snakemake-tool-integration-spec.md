...
That evidence must use an allowed real acceptance `evidenceType` (`real-data-acceptance` or `real-database-acceptance`), reference a completed stored `generated-tool-run-v1` run whose generated workflow runSpec identifies the accepted tool through the current `workflow.nodes[].toolRevisionId`, and have at least one collected non-empty artifact; when `artifactName` is supplied, the artifact must exist in that run result and be non-empty. `real-database-acceptance` evidence must also include `databaseId`, `templateId`, and `role`; the referenced runSpec must bind that role to the same database id and template id under `resourceBindings`, and the remote database registry record for that database id must be `available` and carry the same template id. The real database acceptance script posts that evidence after a generated Snakemake run completes; use `remote_real_database_acceptance.py --rerun-check --keep-production-tools` when the accepted generated smoke tools should remain queryable as production evidence.

WorkflowDesignDraft v1 is the persisted design layer above `generated-tool-run-v1`. The draft contract stores graph nodes by `toolRevisionId` and never stores executable request-side RuleSpec content; the plan-only endpoint resolves saved registry contracts, enforces `workflowReady`, validates graph ports and resources, and previews Snakefile/config assets. The compile endpoint creates an immutable `WorkflowRevision` and returns the submit-ready draft-derived runSpec with `workflowRevisionId`. See `docs/workflow-design-draft-v1.md`.

Tool search, workflow step selection, recommendation, and validation queue UI share `CapabilityGraphSnapshot` as their only read model. The snapshot exposes catalog projections, semantic graph nodes, registered tools, `capability-bundle-v1` summaries, `agentSelectableTools`, selection policy, target acceptance, and prepare queues in one contract. The old `/candidates`, `/candidate-recommendations`, and `/target-acceptance` read endpoints are removed; frontend and agent-facing selection flows must consume `/api/v1/tool-capabilities/capability-graph` and only add workflow steps from tools with a valid `capabilityBundle`. A bundle is selectable only when it has an exact `toolRevisionId`, `WorkflowReady` or `ProductionEnabled` contract state, input/output port schema, resolved environment lock, smoke fixture and expected artifacts, and passed validation evidence.

`capability-bundle-v1` is also the execution audit boundary. Workflow previews, compiled workflow exports, generated run configs, workflow revision manifests, and production evidence events record a bundle audit summary with `capabilityId`, bundle version, exact `toolRevisionId`, tool version, selection rationale, risk, permissions, approval state, validation evidence, environment lock, and next action. Risk and permission fields are not descriptive-only: database or elevated permissions require explicit approval before a bundle can become agent-selectable.
...
| 5. WorkflowDesignDraft builder | MVP wired | Frontend can select workflow-ready tools/databases, bind inputs/params, save/reopen a versioned draft, validate through the plan endpoint, compile/export a WorkflowRevision, preview generated assets, and submit a compiled draft-derived runSpec. | User-created generated-tool runs must reference a saved WorkflowDesignDraft id, revision, and `workflowRevisionId`. Direct generated runSpec shapes are unsupported. |
...
- User-created run requests for this virtual pipeline must carry a `workflowDesign` marker with a saved `draftId` and `revision`, plus a `workflowRevisionId`.
- The runSpec is compiled from that saved draft by the compile/export endpoint after plan validation; request-side `tool`, `ruleTemplate`, or `ruleSpecDraft` content is unsupported.
...
- Each saved node identifies its tool revision with `toolRevisionId` and may use explicit external input bindings under `inputs`, keyed by the tool rule input name:
