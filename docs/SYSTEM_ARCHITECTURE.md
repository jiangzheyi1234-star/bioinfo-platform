...
- execution orchestration 实现在 `core/execution/tool_bridge_execution_orchestrator.py`
- unified result builders 实现在 `core/execution/tool_bridge_result_views.py`
- typed single-tool view 组装继续由 `core/execution/single_tool_view_builder.py` 负责
...
- `tool_bridge_service.py`
  - 工作台后端桥接门面
- `tool_bridge_execution_orchestrator.py`
  - ToolBridge 执行编排 seam
- `tool_bridge_result_views.py` / `single_tool_view_builder.py`
  - unified result builders 与 typed view 组装
