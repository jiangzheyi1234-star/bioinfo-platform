...
class SSHHostKeyTargetRequest(ApiRequest):
    auth_mode: Literal["password_ref", "key_file", "ssh_config", "agent"] | None = None
    ssh_host_alias: str | None = None
    host: str | None = None
    port: int | None = Field(default=None, ge=1, le=65535)
...
class SSHHostKeyScanRequest(SSHHostKeyTargetRequest):
    pass


class SSHHostKeyAcceptRequest(SSHHostKeyTargetRequest):
    confirmation: Literal["trust-ssh-host-key"]
    fingerprintSha256: str = Field(min_length=1)
...
class WorkflowTriggerReadinessWatcherRunOnceRequest(ApiRequest):
    serverId: str | None = None
...
class ArtifactLifecyclePolicySetRequest(ApiRequest):
    serverId: str | None = None
