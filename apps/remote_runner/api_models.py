...
class RunRuleCacheRestorePinPrepareRequest(RemoteRunnerRequest):
...
class RunRuleCacheRestorePinApplyRequest(RemoteRunnerRequest):
...
class RunRuleCacheRestoreStagedFilePrepareRequest(RemoteRunnerRequest):
...
class RunRuleCacheRestoreStagedFileApplyRequest(RemoteRunnerRequest):
...
class RunRuleCacheRestoreFinalOutputPrepareRequest(RemoteRunnerRequest):
...
class RunRuleCacheRestoreFinalOutputApplyRequest(RemoteRunnerRequest):
...
class WorkflowTriggerReadinessWatcherRunOnceRequest(RemoteRunnerRequest):
    confirmation: Literal["run-readiness-watcher-once"]
    limit: int = Field(default=100, ge=1, le=100)
    actor: str | None = None
    reason: str | None = None
...
class ArtifactLifecycleControllerRunOnceRequest(RemoteRunnerRequest):
