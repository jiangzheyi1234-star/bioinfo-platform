...
from .capability_bundle_audit import (
...
from .generated_workflow_asset_plan import plan_generated_workflow_files
from .generated_workflow_plan import (
    GeneratedWorkflowStepPlan,
    plan_generated_workflow_steps,
)
from .rule_action import materialize_rule_asset_plan
from .rule_outputs import (
    output_artifact_flags,
    output_spec_metadata,
    rule_output_metadata,
)
...
from .workflow_resources import (
    ...
    collect_workflow_resource_specs,
)
...
def prepare_generated_tool_workflow(
    ...
    *,
    ...
    workflow_file_plan = plan_generated_workflow_files(generated_steps)

    workflow_resource_config = build_workflow_resource_config(
        cfg,
        workflow_resource_spec=collect_workflow_resource_specs(
            [step.rule_template for step in generated_steps]
        ),
        bindings=dict(run_spec.get("resourceBindings") or {}),
    )
    if require_workflow_ready:
        _validate_capability_bundle_gate(
            generated_steps, resource_context=workflow_resource_config["resources"]
        )
    ...
    final_outputs = {
        name: str(binding["path"]) for name, binding in exposed_outputs.items()
    }
    ...
    config_content = json.dumps(
        {
            ...
            "tool": _config_tool(
                generated_steps[0],
                resource_context=workflow_resource_config["resources"],
            ),
            "workflow": {
                ...
                "steps": [
                    {
                        ...
                        "rule": step.rule_name,
                        "tool": _config_tool(
                            step, resource_context=workflow_resource_config["resources"]
                        ),
                        "inputs": step.inputs,
                        "outputs": {
                            name: str(path) for name, path in step.outputs.items()
                        },
                        "outputSpecs": rule_output_metadata(
                            step.outputs, step.rule_template
                        ),
                        "params": step.params,
    ...
    snakefile_content = render_generated_workflow_snakefile(
        steps=generated_steps,
    ...
    materialize_rule_asset_plan(workflow_file_plan, workflow_dir=workflow_dir)
...
def _config_tool(
    ...
    *,
    ...
    if rule_spec_draft:
        ...
        provenance = _config_rule_provenance(
            rule_spec_draft, step.rule_template, package_spec
        )
        if provenance:
...
def _config_rule_provenance(
    ...
    lock = (
        rule_spec_draft.get("lock")
        if isinstance(rule_spec_draft.get("lock"), dict)
        else {}
    )
    provenance = {
        "source": str(
            rule_spec_draft.get("source") or rule_template.get("source") or ""
        ).strip(),
        ...
        "wrapperIdentifier": str(
            lock.get("wrapperIdentifier") or rule_template.get("wrapper") or ""
        ).strip(),
