...
def normalize_package_identity(*, source: str, package_name: str, version: str, package_spec: str) -> dict[str, str]:
    ...
    if not normalized_spec:
        normalized_spec = f"{source}::{normalized_name}={requested_version}" if requested_version else f"{source}::{normalized_name}"
    ...
    if spec_name and spec_name.lower() != normalized_name.lower():
