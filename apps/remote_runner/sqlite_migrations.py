...
from .agent_run_authorization_schema import migrate_agent_run_authorization_schema
from .agent_session_schema import migrate_agent_session_schema
from .agent_workspace_proof_schema import assert_agent_workspace_proof_schema, migrate_agent_workspace_proof_schema
...
from .sqlite_artifact_migrations import (
    ...
    ensure_result_package_export_byte_state,
    ensure_result_package_export_payload_mode,
    ensure_result_package_export_retired_at,
    ...
    migrate_result_package_byte_state_schema,
    migrate_result_package_payload_mode_schema,
    migrate_result_package_retired_at_schema,
...
from .sqlite_artifact_lifecycle_policy_migrations import (
    ensure_artifact_lifecycle_policies,
...
CURRENT_SCHEMA_MIGRATION_NAME = AGENT_WORKSPACE_PROOF_MIGRATION_NAME
...
def migrate_runtime_schema(connection: sqlite3.Connection) -> None:
    ...
    if version == 15:
        migrate_result_package_retired_at_schema(
            ...
            record_migration=_record_migration,
            version=16,
            name=RESULT_PACKAGE_RETIRED_AT_MIGRATION_NAME,
        )
        version = read_schema_version(connection)
    if version == 16:
        migrate_artifact_lifecycle_policy_schema(
    ...
    if version == 17:
        ...
        version = read_schema_version(connection)
    if version == 18:
        migrate_agent_run_authorization_schema(
            connection,
            record_migration=_record_migration,
            version=19,
        ...
        version = read_schema_version(connection)
    if version == 19:
        migrate_agent_workspace_proof_schema(connection, record_migration=_record_migration)
        return
    if version != 0:
