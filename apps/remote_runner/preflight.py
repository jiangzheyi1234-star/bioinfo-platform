...
from .generated_workflow import _resolve_requested_steps, _resolve_rule_template, _step_tool_request
...
def _preflight_generated_workflow(cfg: RemoteRunnerConfig, run_spec: dict[str, Any]) -> None:
    ...
    for index, step in enumerate(requested_steps):
        step_id = str(step.get("id") or step.get("name") or f"step_{index + 1}").strip()
        if not step_id:
        ...
        try:
            ...
            raise RunPreflightError(str(exc)) from exc
        _preflight_step_inputs(step, known_outputs)
...
def _preflight_step_inputs(step: dict[str, Any], known_outputs: dict[str, set[str]]) -> None:
    ...
    for binding in raw_inputs.values():
        ...
        if not from_step:
            continue
        if from_step not in known_outputs:
        ...
        if output_name not in known_outputs[from_step]:
...
def _preflight_exposed_outputs(run_spec: dict[str, Any], known_outputs: dict[str, set[str]]) -> None:
    ...
    items = raw_outputs.values() if isinstance(raw_outputs, dict) else raw_outputs
    for binding in items:
        if isinstance(binding, str):
            ...
            raise RunPreflightError("WORKFLOW_OUTPUT_BINDING_INVALID")
        if step_id not in known_outputs:
            raise RunPreflightError(f"WORKFLOW_OUTPUT_STEP_UNKNOWN: {step_id}")
        if output_name not in known_outputs[step_id]:
